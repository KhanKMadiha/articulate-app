...
- **Personalised daily passages** — generated by Claude from your job title, industry, career goals, and focus areas
- **Speaking inspirations** — choose speakers you admire; passages are written in their spirit
- **Record and score** — read aloud, record yourself, and get AI feedback on accuracy, fluency, and pace
- **Dyslexia-friendly mode** — wider spacing, shorter sentences, paragraph-by-paragraph reading, Lexend or OpenDyslexic fonts (also shapes AI passage generation)
...
1. **Clone the repo**
   ...
   The clone folder is `articulate-app` on GitHub. If you work from a different local path (e.g. `Madiha/Articulate` on your machine), use that folder name when opening the project — the layout is the same (`client/` and `server/` at the repo root).
